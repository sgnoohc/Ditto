...
if [[ ${OUTPUTFILES} == *"No such"* ]]; then
  ...
  for OUTPUTFILE in ${OUTPUTFILES}; do 
    gfal-copy -p -f -t 4200 file://`pwd`/${OUTPUTFILE} srm://bsrm-3.t2.ucsd.edu:8443/srm/v2/server?SFN=/hadoop/cms/store/user/phchang/ditto_output/${ANALYSIS}_${TAG}/${OUTPUTFILE} --checksum ADLER32
  done
fi
